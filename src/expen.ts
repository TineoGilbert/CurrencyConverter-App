...
class Expenses implements IExpenses{
    ...
    getTota(): string {
        const total = this.getItems().reduce((ac, item) =>{
            return ac += this.convertCurrency(item, this.finalCurrency);
        }, 0);
